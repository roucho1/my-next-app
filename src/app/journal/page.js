...
const days = [
  ...
  },
  {
    ...
    week: 6,
    topics: [
      "database.py 負責定義資料庫 url、engine（連線到哪個資料庫）、SessionLocal（操作資料用）、Base（繼承用）",
      "models.py 負責定義 table 名稱、欄位的 primary_key、index、nullable 等設定",
      "Base.metadata.create_all(bind=engine) 將所有繼承 Base 的 class 建立成資料表",
      "import models 看起來沒在用，但目的是讓 Post 繼承 Base，Base 才知道要建立哪些資料表",
      "posts.db 加進 .gitignore，資料庫檔案不推上 GitHub",
    ],
    note: "engine 代表連線到哪個資料庫，包含資料庫類型和位置。SessionLocal 是操作資料用的，每個請求進來會用它產生 Session 跟資料庫溝通。create_all 已存在的資料表會跳過不重建。",
  },
  {
    day: 37,
    title: "資料庫 CRUD",
    ...
    week: 6,
    topics: [
      "get_db() 用 yield 確保每次請求結束後 Session 一定關閉，每個路由用 Depends(get_db) 注入 Session",
      ".all() 回傳所有符合結果、.first() 回傳第一筆物件沒有則 None、.filter() 加條件、.delete()/.update() 直接操作",
      "新增：db.add() → db.commit() → db.refresh()",
      "修改：直接改物件欄位 → db.commit()，不需要 db.add()，因為查出來的物件已被 SQLAlchemy 追蹤",
      "修改/刪除不直接用 update()/delete() 是避免 id 不存在時無法回 404，先用 first() 確認資料存在",
      "yield 會暫停函式等路由執行完才繼續，return 會立刻結束導致 Session 在路由用到前就被關閉",
    ],
    note: "autocommit=False 所以 db.add() 只放進暫存區，db.commit() 才真正寫入。db.refresh() 是從資料庫重新讀取物件，讓新增後的 id 有值可以回傳。",
  },
  {
    day: 38,
    title: "假資料換真實資料庫",
    ...
    week: 6,
    topics: [
      "Day 37 已完成假資料替換成資料庫查詢",
      "所有 CRUD 路由改用 SQLAlchemy 操作 posts.db",
      "測試 GET/POST/PUT/DELETE 所有路由正常",
    ],
    note: "Day 37 和 Day 38 內容合併完成，假資料全部替換成真實資料庫操作。",
  },
  {
    day: 39,
    title: "部署 FastAPI",
    ...
    week: 6,
    topics: [
      "pip freeze > requirements.txt 記錄套件版本，有新套件時要重新執行更新",
      "Start Command 改成 uvicorn main:app --host 0.0.0.0 --port $PORT",
      "Render 連結 GitHub 後 git push 自動重新部署，Build Command 跑 pip install",
      "Free 方案閒置會 spin down，冷啟動約 50 秒以上",
    ],
    note: "Render 部署後端伺服器（一直跑著的服務），Vercel 部署前端，用途不同。--host 0.0.0.0 允許外部連線，--port $PORT 使用 Render 指定的 port。作品 demo 時要記得說明冷啟動延遲。",
  },
  {
    day: 40,
    ...
    week: 6,
    topics: [
      "修改環境變數 NEXT_PUBLIC_API_URL 由假資料改為 FastAPI",
      "Server Component 直接 fetch FastAPI，專案結構不複雜情況下不需要 Next.js API Route 中間層",
      "遇到頁面一直閃爍問題，可能原因是 Turbopack 不穩定導致 .next 快取異常，刪掉重建就解決了",
    ],
    note: "前端直接打 FastAPI 比透過 Next.js API Route 中間層更簡單，後者適合需要隱藏 API 網址或做權限控制的情況。Turbopack 還不穩定，遇到奇怪問題先刪 .next 是快速解法。",
  },
  {
    day: 41,
    title: "整合測試 & 除錯",
